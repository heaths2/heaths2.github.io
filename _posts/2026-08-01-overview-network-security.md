...
3거점(IDC · 본사 · 지점)을 전용회선으로 묶고, 경계부터 서버까지 계층마다 통제 지점을 둔 구조입니다.

![엔터프라이즈 네트워크 구성도](/assets/img/2026-08-01/network-topology.svg)
_IDC · 본사 · 지점 3거점 구성 — 공개용으로 IP · 기관명 · 거점명은 일반화_

{: .prompt-info }
> **공개용 도면입니다.** 이중화 단수, 존 분리, 서버팜 계층, 망분리, 전용회선 연동 구조는 실제와 같게 유지했고 공인·사설 IP, 제휴 기관 실명, 거점명, 장비 벤더 조합만 일반화했습니다.

읽는 순서는 이렇습니다.

| 구역 | 내용 |
|---|---|
| **IDC** | 경계 보안(FW·IPS 이중화) → 코어 L3 → DMZ / 관리망 / 개발망·운영망 서버팜 |
| **본사** | 코어 스위치 층별 이중화 → 망분리 3종, 인증·보안 서버팜 |
| **지점** | 전용회선 유입 → 업무망 · 인터넷망 분리 |
