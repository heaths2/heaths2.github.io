...
네트워크를 계층별로 다룬 기록입니다. 물리 스위치 설정부터 방화벽, 로드밸런서, 인증서까지 **패킷이 지나가는 순서**로 정리했습니다.
...
```mermaid
flowchart TD
    C["클라이언트"] --> L2["L2 · 스위치<br/>Cisco Catalyst"]
    L2 --> L3["L3 · 게이트웨이 이중화<br/>FHRP"]
    L3 --> FW["경계 방어<br/>FortiGate · OPNsense"]
    FW --> HOST["호스트 방화벽<br/>iptables · firewalld"]
    FW --> L7["L7 · 리버스 프록시<br/>HAProxy · Nginx"]
    L7 --> APP["애플리케이션"]
    HOST --> APP
```
