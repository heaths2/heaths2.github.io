...
로드밸런서는 전용 장비(HW LB)와 소프트웨어(SW LB) 두 갈래입니다. 트래픽 규모가 크지 않은 환경에서 전용 장비는 과투자라 **SW LB인 HAProxy를 쓰기로 정했고**, 이를 플러그인으로 제공하는 **OPNsense를 그 수단으로 선택**했습니다. 방화벽 위에 얹은 게 아니라 HAProxy가 먼저였습니다.
...
| 판단 | 선택 | 이유 | 검토한 대안 |
|---|---|---|---|
| 로드밸런서 | **HAProxy (SW LB)** | 소규모 트래픽에 전용 HW LB는 과투자. SW LB로 충분 | L4/L7 하드웨어 LB(고비용) |
| HAProxy 구동 수단 | **OPNsense** | 플러그인으로 HAProxy를 제공 — 별도 서버를 세우지 않고 운용 | 별도 리눅스 서버에 HAProxy 직접 구성 |
